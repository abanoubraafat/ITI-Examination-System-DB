...
INSERT [dbo].[Question] (Questions_ID, [Degree], [Text_Questions], [Correct_Answer_Text_Questions], [True_or_False_Questions], [Correct_Answer_True_or_False], [Choose_An_Answer_Question], [Correct_Answer_Choose_Question], [Course_Id])values
 ...
 (2, 10, N'What is a Primary key?', N'Unique key and not null constraint.', N'A relation might have multiple foreign keys?', 1, N'The first true object oriented language is: a) C++. b) SmallTalk. c) C#.', N'b', 2),
 (3, 10, N'What is an aliac command ?', N'It is refered in where clause to identify table or column.', N'A relation might have multiple foreign keys?', 1, N'Which of the following is not an SQL command: a) forget. b) select. c) where', N'a', 3),
 (4, 10, N'What is SQL?', N'SQL stands for Structured Query Language.', N'Is alter a DDL command?', 1, N'The primary key can be: a) null. b) not null. c) both null and not null.', N'b', 1),
 (5, 10, N'What is a join?', N'It is a keyword to query data from more tables.', N'Each column can have more than one data type?', 0, N'The primary key can be: a) null. b) not null. c) both null and not null.', N'b', 2),
 (6, 10, N'What is a DB query?', N'It is a code that is written to get information back from the DB.', N'The condition in a where clause can refer to only one value?', 0, N'What is a table joined with itself called: a) Join. b) Outer Join. c) self-join.', N'c', 4)
